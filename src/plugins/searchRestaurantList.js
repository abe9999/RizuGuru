...
export function searchRestaurantList(args) {
    switch (Object.keys(args).length) {
        ...
        case 4:
            // ページネーションテスト
            return InfiniteScrollingTest(args);
        case 9:
            // 店舗一覧画面で絞り込み検索を行った場合
...
// ページネーションテスト
async function InfiniteScrollingTest(args) {
    return new Promise((resolve) => {
        axios.get(`https://func-rizuguru.azurewebsites.net/api/InfiniteScrollingTest?offset=${args.offset}&keyword=${args.keyword}&lat=${args.lat}&lng=${args.lng}`)
            .then((res) => {
                var result = res.data
                resolve(Promise.all(result))
            })
